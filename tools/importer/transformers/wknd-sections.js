 ...
 * Inserts <hr> section breaks (before each non-first section) and Section
 * Metadata blocks (for sections with a style). Section boundaries come from
 * tools/importer/page-templates.json (template "home", 5 sections), each
 * selector verified against migration-work/cleaned.html.
 *
 * Breaks are inserted in beforeTransform (while every section element still
 * exists, before block parsers replace them) using a marker attribute; the
 * Section Metadata block is inserted in afterTransform anchored to that marker.
